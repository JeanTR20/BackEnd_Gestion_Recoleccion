import { IsString, IsNotEmpty, IsIn, IsOptional, Min, IsNumber } from 'class-validator';

export class CreateUsuarioAdministradorDto{

    @IsNumber({}, {message: 'La variable id_usuario debe ser numerico'})
    @IsNotEmpty({message: 'La variable id_usuario no debe estar vacio'})
    @Min(1, {message: 'La variable debe id_usuario no debe ser menor a 1'})
    id_usuario: number;
